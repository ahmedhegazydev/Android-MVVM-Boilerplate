...
object CubitFlutterStrategy : FlutterMvvmStrategy {
    ...
    override val stateManagement =
        CleanArchitectureConfig.StateManagement.CUBIT
    ...
    override fun generateFeature(project: Project, config: CleanArchitectureConfig) {
        ...
        modelDir.createFileIfNotExists(
            ...
            CubitTemplates.domainModel(featurePascal, featureSnake)
        )

        repoDir.createFileIfNotExists(
            "${featureSnake}_repository.dart",
            CubitTemplates.domainRepository(featurePascal, featureSnake)
        )

        useCaseDir.createFileIfNotExists(
            "get_${featureSnake}_list_usecase.dart",
            CubitTemplates.useCase(featurePascal, featureSnake)
        )

        remoteDir.createFileIfNotExists(
            "${featureSnake}_api_service.dart",
            CubitTemplates.apiService(featurePascal, featureSnake)
        )

        repoImplDir.createFileIfNotExists(
            "${featureSnake}_repository_impl.dart",
            CubitTemplates.repositoryImpl(featurePascal, featureSnake)
        )

        vmDir.createFileIfNotExists(
            "${featureSnake}_state.dart",
            CubitTemplates.state(featurePascal, featureSnake)
        )

        vmDir.createFileIfNotExists(
            "${featureSnake}_cubit.dart",
            CubitTemplates.cubit(featurePascal, featureSnake)
        )

        uiDir.createFileIfNotExists(
            "${featureSnake}_screen.dart",
            CubitTemplates.screen(featurePascal, featureSnake, config.di)
        )
    }
}
